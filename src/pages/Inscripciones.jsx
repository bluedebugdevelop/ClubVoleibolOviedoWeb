...
export default function Inscripciones() {
  ...
  async function envia(e) {
    ...
    try {
      ...
      if (r.ok && cuerpo.ok === true) {
        ...
        // le da a la familia el correo del club
        setEstado('sinConectar')
      } else {
        setEstado('error')
      }
    } catch (err) {
      // sin red o servidor caído: se avisa con el correo del club, pero el
      // motivo queda en consola para poder diagnosticarlo
  ...
  return (
    <>
      ...
      {/* El formulario va lo primero, nada más pasar la cabecera: quien entra
          aquí entra a apuntarse, no a leer (03-08-2026). «Cómo funciona» y «Qué
          llevar» bajan después, que es información de apoyo.
...
/**
 * Formulario propio de la web, el que manda el correo con `api/inscripcion.js`.
 *
 * Hoy no se pinta: las inscripciones van al Google Form del club. Se conserva
 * entero y vuelve solo con poner `formularioInscripcionUrl` a null.
 */
function FormularioPropio({ datos, estado, consentimiento, cambia, envia, setConsentimiento }) {
  return (
    <form className="form inscripcion" onSubmit={envia}>
      ...
      <div className="campos">
        ...
        {/* este teléfono es el de quien se apunta, no el del club: se queda */}
        <div className="field">
